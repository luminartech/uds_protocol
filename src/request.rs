...
use crate::{
    Decode, DiagnosticDefinition, Error, NegativeResponseCode, ReadDTCInfoRequest, ResetType,
    ...
    services::{
        ...
        RequestDownloadRequest, RoutineControlRequest, SecurityAccessRequest,
        SecurityAccessRequestTx, TesterPresentRequest, TransferDataRequest, TransferDataRequestTx,
    ...
    fn decode<R: Read>(reader: &mut R) -> Result<Self, Error> {
        ...
        Ok(match service {
            UdsServiceType::CommunicationControl => {
                Self::CommunicationControl(<CommunicationControlRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::ControlDTCSettings => {
                Self::ControlDTCSettings(<ControlDTCSettingsRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::DiagnosticSessionControl => {
                Self::DiagnosticSessionControl(<DiagnosticSessionControlRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::EcuReset => Self::EcuReset(<EcuResetRequest as SingleValueWireFormat>::decode(reader)?),
            UdsServiceType::ReadDataByIdentifier => {
                Self::ReadDataByIdentifier(<ReadDataByIdentifierRequest<D::DID> as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::ReadDTCInfo => Self::ReadDTCInfo(<ReadDTCInfoRequest as SingleValueWireFormat>::decode(reader)?),
            UdsServiceType::RequestDownload => {
                Self::RequestDownload(<RequestDownloadRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::RequestTransferExit => Self::RequestTransferExit,
            UdsServiceType::RoutineControl => {
                Self::RoutineControl(<RoutineControlRequest<D::RID, D::RoutinePayload> as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::SecurityAccess => {
                Self::SecurityAccess(<SecurityAccessRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::TesterPresent => {
                Self::TesterPresent(<TesterPresentRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::TransferData => {
                Self::TransferData(<TransferDataRequest as SingleValueWireFormat>::decode(reader)?)
            }
            UdsServiceType::WriteDataByIdentifier => {
                Self::WriteDataByIdentifier(<WriteDataByIdentifierRequest<D::DiagnosticPayload> as SingleValueWireFormat>::decode(reader)?)
            }
...
// ---------------------------------------------------------------------------
// no_std RX request enum (zero-copy, no DiagnosticDefinition needed)
// ---------------------------------------------------------------------------

/// Zero-copy RX request. Borrows from the wire buffer.
///
/// Unlike [`Request<D>`], this enum does not require a [`DiagnosticDefinition`]
/// generic parameter — variable-length payloads are stored as raw `&'a [u8]`
/// slices.
#[derive(Clone, Debug)]
#[non_exhaustive]
pub enum RequestRx<'a> {
...
impl<'a> Decode<'a> for RequestRx<'a> {
